...
guided try/demo path, graphics doctor, combat-stage parity evidence, isolated
fixed-seed full run, post-run visual report, optional Chinese
try/demo/status/Codex/run-report review surface, and completion audit.
`venv312/bin/python scripts/acceptance_check.py` runs that mock-first review
path plus the blocked-home try storage fallback as one concise report and still
leaves `SIGN-OFF: pending`.
...
| --- | --- | --- | --- |
| User satisfaction | Agent playtest is recorded and `docs/engineering/USER_ACCEPTANCE_20260601.md` exists as a pending template, but user acceptance is not yet provided; `signoff_check.py` prints acceptance commands for the review path. The path can be prechecked with `scripts/acceptance_check.py`, `release_check.py --doctor-only`, and `release_check.py --combat-stage-only` without changing the sign-off marker. | `docs/engineering/USER_ACCEPTANCE_20260601.md` contains `SIGN-OFF: accepted`. | Run the acceptance commands, play or explicitly accept the current build, then update the template sign-off line. |
| License | MIT selected on 2026-06-11; `docs/engineering/LICENSE_DECISION_20260601.md` records the decision, `LICENSE` exists, `pyproject.toml` says `MIT`, and README/README.zh link the root license file. | `signoff_check.py --json` reports `license: READY`. | No action required unless the release policy changes. |
